...
export class DevPanel {
  ...
  constructor(
    ...
    const r1 = 14, r2 = 38, r3 = 62, r4 = 86, r5 = 116;
    ...
    // Row 4: season jump buttons (4 × 67px, 4px gap, fills panel width)
    ...
    for (let i = 0; i < 4; i++) {
      ...
      this.mkBtn(scene, sBtnW / 2 + i * (sBtnW + 4) + 0.5, r4, sBtnW, color, label, textColor, () => onJumpSeason(label));
    }

    // Row 5: git hash | clock | Reset All | fps
    this.panel.add(
      scene.add.text(8, r5, `#${__GIT_HASH__}`, { fontSize: '11px', color: '#334455', fontFamily: UI_FONT }).setOrigin(0, 0.5),
    );

    this.clockText = scene.add
      .text(PANEL_W / 2 - 36, r5, '', { fontSize: '12px', color: '#aaccff', fontFamily: UI_FONT })
      .setOrigin(0.5);
    this.panel.add(this.clockText);

    const resetBg = scene.add
      .rectangle(PANEL_W / 2 + 56, r5, 96, 22, 0x440000)
      .setInteractive({ useHandCursor: true });
    this.panel.add(resetBg);
    this.panel.add(
      scene.add.text(PANEL_W / 2 + 56, r5, 'Reset All', { fontSize: '12px', color: '#ff8888', fontFamily: UI_FONT }).setOrigin(0.5),
    );
    ...
    this.fpsText = scene.add
      .text(PANEL_W - 4, r5, '', { fontSize: '12px', color: '#ffdd88', fontFamily: UI_FONT })
      .setOrigin(1, 0.5);
