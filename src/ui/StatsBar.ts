...
export class StatsBar {
  ...
  constructor(scene: Phaser.Scene, panelTop: number, width: number) {
    ...
    this.taxRateText = scene.add
      .text(8, midY, '', { fontSize: '15px', color: '#88ccff', fontFamily: UI_FONT })
      .setOrigin(0, 0.5)
      .setDepth(11);
    this.goldText = scene.add
      .text(width - 8, midY, '', { fontSize: '15px', color: '#ffd966', fontFamily: UI_FONT })
      .setOrigin(1, 0.5)
      .setDepth(11);
  }

  update(gold: number, taxRate: number): void {
    this.goldText.setText(`Balance: ${fmtBalance(gold)}`);
    this.taxRateText.setText(`Income: ${fmt(taxRate * 3600)}/hr`);
  }
